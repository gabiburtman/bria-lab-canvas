...
const HistoryPanel = ({
  ...
  return <>
      ...
      {isCollapsed && <div className="w-16 h-full bg-lab-surface rounded-lg shadow-lg flex flex-col relative" onMouseEnter={handleMouseEnter}>
        ...
        handleCollapse(false, true);
          ...
          <div className="flex-1 flex flex-col min-h-0">
            {history.length === 0 ? <div className="flex-1 flex items-center justify-center">
                ...
                <div className="flex flex-col gap-4 px-2 flex-1 overflow-y-auto items-center pb-4 pt-2">
                  {history.slice(0, 4).map(item => <div key={item.id} onClick={() => {
              onItemClick(item);
              handleCollapse(false, true);
            }} className={cn("w-12 h-12 rounded cursor-pointer transition-all duration-200 border-2 flex-shrink-0", activeId === item.id ? "border-lab-border-focus shadow-lab-glow-focus" : "border-lab-border hover:border-lab-border-focus hover:shadow-lab-glow-subtle")}>
                      {item.thumbnail ? <img src={item.thumbnail} alt="Experiment" className="w-full h-full rounded object-cover" /> : <div className="w-full h-full rounded bg-lab-border flex items-center justify-center">
      ...
      {!isCollapsed && <div className="absolute top-0 right-0 h-full w-80 bg-lab-surface rounded-lg shadow-2xl flex flex-col z-50 border border-lab-border" onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
          ...
          <ScrollArea className="flex-1">
            <div className="px-6 pb-6">
              {history.length === 0 ? <div className="text-center py-12">
                  ...
                  {history.map(item => <div key={item.id} onClick={() => handleItemClick(item)} className={cn("p-2 border border-lab-border rounded-lg cursor-pointer transition-all duration-200 hover:shadow-lab-glow-subtle", activeId === item.id ? "bg-lab-primary/10 border-lab-border-focus shadow-lab-glow-focus" : "bg-lab-surface hover:bg-lab-interactive-hover")}>
                      {item.thumbnail ? <img src={item.thumbnail} alt="Experiment" className="w-full aspect-square rounded-md object-cover" /> : <div className="w-full aspect-square rounded-md bg-lab-border flex items-center justify-center">
